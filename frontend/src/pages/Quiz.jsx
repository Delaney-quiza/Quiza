  ...
  return (
    <div style={{background:"#111113",border:`1px solid ${expired?"#4e0606":low?"#4e3a06":"#222"}`,borderRadius:16,padding:24,animation:"fadeScale .5s ease both",transition:"border-color .5s"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:20}}>
        {[0,1,2].map(i => <div key={i} style={{flex:1,height:3,borderRadius:3,background:i<qi?"#22c55e":i===qi?"#3b82f6":"#1a1a1e"}}/>)}
      </div>
      ...
      <div style={{display:"grid",gap:10}}>
        {q.options.map((opt, i) => (
          <button key={i} onClick={() => !expired && onAnswer(i)} disabled={expired}
            style={{background:"#1a1a1e",border:"1px solid #2a2a2e",borderRadius:12,padding:"16px 18px",color:expired?"#333":"#ccc",fontFamily:S.mono,fontSize:14,textAlign:"left",lineHeight:1.4,display:"flex",alignItems:"center",gap:12,transition:"all .2s",cursor:expired?"default":"pointer"}}>
            <span style={{width:28,height:28,borderRadius:8,background:"#0d0d0f",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#555",fontWeight:700,flexShrink:0}}>
    ...
    <div style={{background:"#111113",border:`1px solid ${correct?"#1a3a2a":"#2a1a1a"}`,borderRadius:16,padding:24,animation:"fadeScale .4s ease both"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        ...
        <div>
          ...
          <div style={{fontFamily:S.mono,fontSize:12,color:"#555"}}>
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (todayData.played) {
          // Build a result object from the saved data
          const r = todayData.result;
          setResult({
            correct: [!!r.question_1_correct, !!r.question_2_correct, !!r.question_3_correct],
            correct_count: (r.question_1_correct ? 1 : 0) + (r.question_2_correct ? 1 : 0) + (r.question_3_correct ? 1 : 0),
            score: r.score,
  ...
  return (
    <div style={{maxWidth:520,margin:"0 auto",padding:"24px 20px 48px",minHeight:"100vh",background:"#08080a",fontFamily:S.mono}}>
      ...
      <div style={{textAlign:"center",marginBottom:24,animation:"slideUp .4s ease both"}}>
        ...
        <p style={{color:"#444",fontSize:11,margin:0,textTransform:"uppercase",letterSpacing:3}}>
      ...
      {stats && (
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",marginBottom:8,animation:"slideUp .4s ease .1s both"}}>
          ...
          <button onClick={() => setShowBadges(true)} style={{background:"#1a1a1e",border:"1px solid #2a2a2e",borderRadius:100,padding:"8px 16px",color:"#aaa",fontSize:12,display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}>
      ...
      {phase === "error" && (
        <div style={{textAlign:"center",padding:"60px 0",animation:"fadeScale .5s ease both"}}>
          ...
          <p style={{color:"#666",fontSize:13,maxWidth:320,margin:"0 auto"}}>{error === "Invalid player token" ? "Your session expired. Tap Try Again to start fresh!" : (error || "Something went wrong. Try refreshing.")}</p>
      ...
      {phase === "ready" && questions.length > 0 && (
        <div style={{textAlign:"center",padding:"40px 0",animation:"fadeScale .5s ease both"}}>
          ...
          <p style={{color:"#666",fontSize:13,lineHeight:1.7,margin:"0 0 8px",maxWidth:320,marginLeft:"auto",marginRight:"auto"}}>
            3 questions. {TIME_LIMIT} seconds each.
          </p>
          <p style={{color:"#555",fontSize:12,margin:"0 0 32px"}}>Answer faster for more points. Max 100 pts per question.</p>
      ...
      {/* === PLAYING === */}
      {phase === "playing" && q && (
        <ActiveQuestion question={q} qi={currentQ} onAnswer={handleAnswer} timeLeft={timeLeft} max={TIME_LIMIT}/>
      )}
        ...
        <>
          <div style={{background:"linear-gradient(135deg,#0a0a0c,#111118)",border:"1px solid #222",borderRadius:20,padding:32,textAlign:"center",animation:"fadeScale .6s ease both",marginTop:16}}>
            <div style={{fontSize:48,marginBottom:8}}>
              {result.correct_count === 3 ? "🏆" : result.correct_count >= 2 ? "💪" : result.correct_count >= 1 ? "🤔" : "😅"}
            </div>
            ...
            <p style={{color:"#666",fontSize:13,margin:"0 0 24px"}}>
              {result.correct_count}/3 correct · {result.score} points
            </p>

            {/* Score breakdown - only render if questions are loaded */}
            {questions.length > 0 && result.correct && result.correct.map((isCorrect, i) => (
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"#0d0d0f",borderRadius:12,marginBottom:8,border:`1px solid ${isCorrect?"#1a3a2a":"#2a1a1a"}`,animation:`slideUp .4s ease ${i*.1}s both`,textAlign:"left"}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:"#555",textTransform:"uppercase",letterSpacing:.5}}>
                    Question {i+1}
                  </div>
                </div>
              </div>
...
{`I played QuiZa today and scored ${result.score}/300. Can you do better?
...
Play Now → quiza.co.za`}
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
              <button onClick={shareWhatsApp} style={{
                background:"#25D366",color:"#fff",border:"none",borderRadius:100,
                padding:"14px 24px",fontSize:13,fontWeight:700,cursor:"pointer",
                display:"flex",alignItems:"center",gap:8,letterSpacing:.3
              }}>
              ...
              <button onClick={shareX} style={{
                background:"#000",color:"#fff",border:"1px solid #333",borderRadius:100,
                padding:"14px 24px",fontSize:13,fontWeight:700,cursor:"pointer",
                display:"flex",alignItems:"center",gap:8,letterSpacing:.3
              }}>
              ...
              <button onClick={shareFacebook} style={{
                background:"#1877F2",color:"#fff",border:"none",borderRadius:100,
                padding:"14px 24px",fontSize:13,fontWeight:700,cursor:"pointer",
                display:"flex",alignItems:"center",gap:8,letterSpacing:.3
              }}>
            ...
            <button onClick={handleCopy} style={{
              background:"none",color:"#555",border:"none",padding:"12px",
